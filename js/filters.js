/**
 * Système de filtres enrichi : motorisations avec puissances,
 * codes défauts par modèle/motorisation, campagnes de rappel, TSB
 */
...
class FilterManager {
  ...
  populateBrandSelect() {
    const s = document.getElementById('filterBrand');
    if (!s) return;
  ...
  populateComponentSelect() {
    ...
    if (!s) return;
    [...new Set(getComponentsByType())].sort().forEach(c => {
  ...
  attachEventListeners() {
    ...
    if (model) model.addEventListener('change', e => this.updateEngineSpecSelect(e.target.value));
  ...
  updateModelSelect(brand) {
    ...
    if (brand) {
      getModelsByBrand(brand).forEach(m => {
  ...
  updateEngineSpecSelect(model) {
    ...
    if (brand && model) {
      getEnginesByModel(brand, model).forEach(e => {
        ...
        o.value = e.name;
        o.textContent = `${e.name} (${e.ch} ch / ${e.kw} kW) - ${e.years}`;
        specSelect.appendChild(o);
      });
      specSelect.disabled = getEnginesByModel(brand, model).length === 0;
  ...
  updateComponentSelectByType(type) {
    ...
    [...new Set(getComponentsByType(type))].sort().forEach(c => {
      const o = document.createElement('option'); o.value=c; o.textContent=c; s.appendChild(o);
    });
  }
  ...
  resetFilters() {
    ['filterBrand','filterModel','filterEngineSpec','filterEngine','filterType','filterComponent','filterCode','filterSearch']
  ...
  getSelectedEngine() {
    ...
    return getEnginesByModel(brand, model).find(e => e.name === engineSpec) || null;
  }
  ...
  renderEngineInfo() {
    ...
    let html = `
      <p><strong>Moteur :</strong> ${eng.name} | <strong>Type :</strong> ${typeLabels[eng.type]} |
         <strong>Puissance :</strong> ${eng.ch} ch (${eng.kw} kW) | <strong>Code moteur :</strong> ${eng.code} |
         <strong>Années :</strong> ${eng.years}</p>`;
    if (eng.issues.length) {
      html += `<h3>⚠️ Problèmes connus / répertoriés</h3><ul>${eng.issues.map(i=>`<li>${i}</li>`).join('')}</ul>`;
    }
    if (eng.recalls.length) {
      html += `<h3>📢 Campagnes / rappels applicables</h3><ul>${eng.recalls.map(r=>`<li>${r}</li>`).join('')}</ul>`;
    }
    ...
    list.innerHTML = `<h3 style="margin:0.5rem 0;">📢 Campagnes de rappel & bulletins techniques (${camps.length})</h3>` +
      camps.map(c => `
        <div class="campaign-card">
          <strong>${sev[c.severity]||''} ${c.title}</strong>
          <p style="margin:0.25rem 0;"><small>${c.brands.join(', ')} - ${c.models.slice(0,6).join(', ')}${c.models.length>6?'...':''} | ${c.engines.join(', ')} | ${c.years}</small></p>
          <p style="margin:0;"><strong>Action :</strong> ${c.action}</p>
        </div>`).join('');
  }
  ...
  async renderResults() {
    ...
    const filtered = this.filterFaults(allFaults);

    resultsList.innerHTML = '';
    resultsCount.textContent = filtered.length;
    if (!filtered.length) { noResults.style.display = 'block'; return; }
    noResults.style.display = 'none';
    filtered.forEach(f => resultsList.appendChild(this.createFaultCard(f)));
  }
  ...
  filterFaults(faults) {
    ...
    return faults.filter(f => {
      ...
      if (this.currentFilters.model) {
        if (f.modelRefs && f.modelRefs.length && !f.modelRefs.includes(this.currentFilters.model)) {
          if (!(f.brands.includes('ALL'))) return false;
        }
      }
      if (eng) {
        const strong = f.engineRefs && f.engineRefs.includes(eng.name);
        const loose = f.engines.includes(eng.type);
        if (!strong && !loose) return false;
  ...
  createFaultCard(fault) {
    ...
    card.innerHTML = `
      ...
      <div class="meta">
        ...
        ${hasTsb ? '<span class="tag" style="background:#fff7ed;border-color:#fed7aa;">📢 TSB/Rappel</span>' : ''}
      </div>`;
    return card;
  }
